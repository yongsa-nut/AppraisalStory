...
# Choose relevant domain (un-comment for different domain)
...
```{r spearman correlation CI}
# This function is used to calculate the confidence intervals for the 
# Spearman rank correlation following recommendations from Bonnett and Wright (2000)

# Function is adapted from https://stats.stackexchange.com/questions/18887/how-to-calculate-a-confidence-interval-for-spearmans-rank-correlation user retodomax - all credit is theirs.

spearman_CI <- function(correlation_coefficient, n = 80, alpha = 0.05){
  
  CI <- sort(tanh(atanh(correlation_coefficient) + c(-1,1)*sqrt((1+correlation_coefficient^2/2)/(n-3))*qnorm(p = alpha/2))) # use function from Bonnett and Wright (2000) and sort by size (note c(-1,1) ensures we calculate both lower and upper bound)
  
  return(CI) # returns vector CI
}
...
print_tibble <- tibble(`Model 1` = NULL, `Model 2` = NULL, Domain = NULL, print_friendly = NULL) # initialise print version for latex tables.
...
correlation_results <- combn(names(cor_data_prep), 2, simplify = FALSE) %>% # make all combinations of model names
  map_df(~{ # take relevant row in data_prep that corresponds to a pair of names
    ...
    tibble( # create new tibble with results from test
      ...
      n = length(x) # number of means correlated
    )
  ...
  rowwise() %>% 
  mutate(CI_low = spearman_CI(correlation_coefficient = correlation, n = n, alpha = 0.0001)[1], # use function above to calculate CI - based on Bonnett and Wright (2000)
         CI_high = spearman_CI(correlation_coefficient = correlation, n = n, alpha = 0.0001)[2],
         print_friendly = paste(round(correlation, 2), " [", round(CI_low, 2), ", ", round(CI_high, 2), "]", sep = "")) # create a string with correlation coefficient + CI for tables (round to two decimals and add standard formatting) 

 # for tables with all correlations and CIs
print_tmp <- correlation_results %>% 
  mutate(Domain = domain_tmp) %>%
  select(`Model 1`, `Model 2`, Domain, print_friendly)

print_tibble <- bind_rows(print_tibble, print_tmp)
...
print_tibble <- tibble(`Domain 1` = NULL, `Domain 2` = NULL, Model = NULL, print_friendly = NULL) # initialise print version for latex tables.
...
# for tables with all correlations and CIs
print_tmp <- correlation_results %>% 
  mutate(Model = model_tmp) %>%
  select(`Domain 1`, `Domain 2`, Model, print_friendly)
...
print_tibble <- tibble(`Prompt 1` = NULL, `Prompt 2` = NULL, Model = NULL, print_friendly = NULL) # initialise print version for latex tables.
...
print_tmp <- correlation_results %>% 
  mutate(Model = model_tmp) %>%
  select(`Prompt 1`, `Prompt 2`, Model, print_friendly)
...
plot_emotion_diff <- wide_emotion_differences %>%
  pivot_longer(cols = c("Human vs claude-sonnet", "Human vs gemini-pro", "Human vs GPT-4o", "Human vs llama"),
               names_to = "models", values_to = "mean_diff") %>%
  mutate(emotion = as.factor(emotion),
  ...
  scale_y_discrete(limits = rev(emotion_order), labels = rev(emotion_order))

plot_emotion_diff %>% show()

differences_total <- wide_emotion_differences %>%
  pivot_longer(cols = c("Human vs claude-sonnet", "Human vs gemini-pro", "Human vs GPT-4o", "Human vs llama"),
               names_to = "models", values_to = "mean_diff") %>%
  mutate(emotion = as.factor(emotion),
         models = as.factor(models)) %>%
  group_by(models) %>%
  summarise(total_diff = sum(mean_diff))
